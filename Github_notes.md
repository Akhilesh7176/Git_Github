...
Git is a version control system. To explain in laymen terms, it is like storage for code. And Developers can contribute to a single codebase. Codebase Reviews Merging and Many can be done.\
...
This initialises the git into the folder and it has the track of the folder and the code changes.\
...
It gives commit hash key usually the code for directly seeing the commit at any point of time because if i belive many changes happen and whenever any website or product gets a bug or a problem to roleout to the previous version they need the exact commit or previous build so that can roll back.\
